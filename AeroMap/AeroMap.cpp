...
AeroMapApp::AeroMapApp(int& argc, char** argv)
	...
	, m_ViewType(ViewType::None)
{
	...
	{
		Logger::Write(__FUNCTION__, "'OdmLib' path not found (from '%s')", argv[0]);
		QMessageBox::warning(nullptr, APP_NAME, "OdmLib folder not found. Photogrammetry process will not run.", QMessageBox::StandardButton::Ok);
	}
}

AeroMapApp::~AeroMapApp()
{
}

void AeroMapApp::LoadSettings()
{
	QSettings settings(ORG_KEY, APP_KEY);
	ms_AppDataPath = XString(settings.value(PATH_APPDATA_KEY, ms_AppDataPath.c_str()).toString());
}

void AeroMapApp::ActivateView(AeroMapApp::ViewType view, int subItem)
{
	m_ViewType = view;
...
AeroMapApp::ViewType AeroMapApp::GetActiveView()
{
	return m_ViewType;
}

MainWindow* AeroMapApp::GetMainWindow()
{
...
XString AeroMapApp::GetEnvValue(const char* envKey)
{
...
void AeroMapApp::LogWrite(const char* text, ...)
{
	char buf[255] = { 0 };
...
void AeroMapApp::SetComboByUserData(QComboBox* cbo, int userData)
{
...
void AeroMapApp::DumpRaster(GDALDataset* pDS, const char* fileName)
{
...
void AeroMapApp::DumpVector(GDALDataset* pDS, const char* fileName)
{
	// Debugging function to dump the contents of a dataset.
	//
	// Inputs:
...
XString AeroMapApp::ResolveAppDataPath()
{
...
XString AeroMapApp::ResolveOdmLibPath(int& argc, char** argv)
{
	// Starting at location of exe, traverse up until "OdmLib"
	// found; this ok for dev, but an install may need different
	// approach
	...
	if (argc > 0)
	{
		XString cur_path = XString(argv[0]).GetPathName();
		
		QDir dir(cur_path.c_str());
		while (dir.isRoot() == false)
		{
			XString path = XString::CombinePath(cur_path, "OdmLib");
