	...
	, mb_PaintMask(false)
{
	...
	QDockWidget* pDockProp = new QDockWidget(tr("Properties"), this);
	pDockProp->setAllowedAreas(Qt::LeftDockWidgetArea);					// can only dock on left
	mp_PropWindow = new PropWindow(pDockProp);							// create widget with dock as parent
	pDockProp->setWidget(mp_PropWindow);								// then set the widget (seems redundant)
	addDockWidget(Qt::LeftDockWidgetArea, pDockProp);					// add it to the main window
	mp_PropWindow->hide();												// not show by default
...
void MainWindow::CreateActions()
{
	...
	mp_actionViewProp = new QAction(QIcon(""), tr("Property Window"), this);
	mp_actionViewProp->setStatusTip(tr("Show/hide the property window"));
	mp_actionViewProp->setCheckable(true);
	verify_connect(mp_actionViewProp, SIGNAL(triggered()), this, SLOT(OnViewProp()));
...
void MainWindow::CreateMenus()
{
	...
	mp_menuView->addAction(mp_actionViewProp);
...
void MainWindow::LoadProject(const char* pathName)
{
	...
	//OnViewChanged(AeroMapApp::ViewType::Drone);
...
void MainWindow::OnViewProp()
{
	if (mp_PropWindow->isVisible())
		mp_PropWindow->hide();
	else
		mp_PropWindow->show();
}
...
void MainWindow::OnViewMenuShow()
{
	...
	mp_actionViewProp->setChecked(mp_PropWindow->isVisible());
...
void MainWindow::OnViewChanged(AeroMapApp::ViewType view, int subItem)
{
	...
	case AeroMapApp::ViewType::Drone:
		...
		break;
	case AeroMapApp::ViewType::Lidar:
		{
