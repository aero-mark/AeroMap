...
{
	QDir dir(ms_DroneInputPath.c_str());
...
{
	return XString::CombinePath(tree.odm_georef, "coords.txt");
}
...
void Project::InitArg()
{
	...
	//					the areas that have been filled. 
	//					Default: False
	...
	//					faster and provides similar results in planar areas. 
	//					Default: False
	...
	// --skip-report    Skip generation of PDF report. This can save time if you don't need a report. 
	//					Default: False
...
void Project::InitTree()
{
	...
	tree.odm_textured_model_glb = "odm_textured_model_geo.glb";

	tree.odm_georef = XString::CombinePath(ms_DroneOutputPath, "odm_georeferencing");
	tree.odm_georef_model_laz = XString::CombinePath(tree.odm_georef, "odm_georeferenced_model.laz");
	...
	XString odm_lib = GetApp()->GetOdmLibPath();

	// be sure to have extensions - qt process can be picky about how it resolves these
	tree.prog_opensfm = XString::CombinePath(odm_lib, "bin/opensfm/bin/opensfm.bat");
	tree.prog_densify = XString::CombinePath(odm_lib, "bin/OpenMVS/DensifyPointCloud.exe");
	tree.prog_filter = XString::CombinePath(odm_lib, "bin/FPCFilter.exe");
	tree.prog_pdal = XString::CombinePath(odm_lib, "bin/pdal.exe");
	tree.prog_poisson = XString::CombinePath(odm_lib, "bin/PoissonRecon.exe");
	tree.prog_recon_mesh = XString::CombinePath(odm_lib, "bin/OpenMVS/ReconstructMesh.exe");
	tree.prog_recon_tex = XString::CombinePath(odm_lib, "bin/texrecon.exe");
	tree.prog_dem2mesh = XString::CombinePath(odm_lib, "bin/dem2mesh.exe");
	tree.prog_dem2points = XString::CombinePath(odm_lib, "bin/dem2points.exe");
	tree.prog_odm_orthophoto = XString::CombinePath(odm_lib, "bin/odm_orthophoto.exe");
	tree.prog_gdal_buildvrt = XString::CombinePath(odm_lib, "venv/Lib/site-packages/osgeo/gdalbuildvrt.exe");
	tree.prog_gdal_translate = XString::CombinePath(odm_lib, "venv/Lib/site-packages/osgeo/gdal_translate.exe");
}
