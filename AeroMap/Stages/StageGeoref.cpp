...
int StageGeoref::Run()
{
	...
	if ((AeroLib::FileExists(tree.odm_georef_model_laz) == false) || Rerun())
	{
		...
		if (georef.is_valid)
		{
			...
			if (pFile)
			{
				...
				fprintf(pFile, "        \"filename\": \"%s\",\n", tree.odm_georef_model_laz.c_str());
	...
	{
		Logger::Write(__FUNCTION__, "Found existing georeferenced model: '%s'", tree.odm_georef_model_laz.c_str());
	}
