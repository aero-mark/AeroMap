...
int StageMesh::CreateMesh3D()
{
	...
	if (arg.skip_3dmodel == false)
	{
		...
		{
			Logger::Write("Found existing mesh file: '%s'", tree.odm_mesh.c_str());
		}
	}
...
int StageMesh::CreateMesh25D()
{
	...
	{
		if ((AeroLib::FileExists(tree.odm_25dmesh) == false) || Rerun())
		{
			Logger::Write(__FUNCTION__, "Writing 2.5D mesh file: '%s'", tree.odm_25dmesh.c_str());
			...
			Mesh::create_25dmesh(tree.filtered_point_cloud, tree.odm_25dmesh, radius_steps, dsm_resolution, arg.octree_depth, samples_per_node, 
				arg.mesh_size, arg.max_concurrency, method, true, max_tiles);
		}
		else
		{
			Logger::Write("Found existing 2.5D mesh file: '%s'", tree.odm_25dmesh.c_str());
		}
	}
